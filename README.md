<!--
  Hey there, I'm Michael Bowen!
  Thanks for stopping by and checking out my profile README.

  Computer Science student (May 2026) • U.S. Marine Corps veteran
-->

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0E7490&height=120&section=header" alt="header"/>

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.herokuapp.com?font=Montserrat&weight=500&size=25&duration=4500&pause=500&color=0E7490&width=750&lines=Welcome!+I%E2%80%99m+Michael+Bowen;Computer+Science+Student+%7C+May+2026;Backend+%26+Infrastructure+Projects" alt="Typing SVG"/>
</a>

### About me
I’m a Computer Science student graduating in **May 2026** and a **U.S. Marine Corps veteran**. I enjoy building practical software projects—usually a mix of backend development, automation, and working in Linux environments.

---

### Main skills
[![My Skills](https://skillicons.dev/icons?i=python,java,js,c,cpp,cs,linux,bash,git,github,flask,fastapi,spring,nginx,ansible,kubernetes,mysql,selenium,playwright)](https://skillicons.dev)

**Other tools:** Slurm, Open OnDemand (OIDC), OpenTofu, Fiddler, TLS, Assembly (x86/AT&T)

---

### Featured projects
- **[personal-vpn](https://github.com/michaelj-tech1/personal-vpn)**  
  OpenVPN infrastructure across 20 global servers with automation for provisioning and updates.

- **[battlenet-registration-automation](https://github.com/michaelj-tech1/battlenet-registration-automation)**  
  Multithreaded Python tool with a PySide6 GUI that executes a multi-step HTTP workflow via direct requests.

- **Private API services**  
  I also build small Flask/FastAPI services for automation and integrations. Most of that code is private since it’s tied to live systems, but I’m happy to discuss the design and deployment approach.

---

### Connect with me!
<div>
  <a href="https://www.linkedin.com/in/michael-bowen-287558210/">
    <img src="https://github.com/user-attachments/assets/880aaea6-79b9-4058-b9b4-342391ca04ea" alt="LinkedIn" width="35" height="35"/>
  </a>
  <a href="mailto:michael.bowen1289@gmail.com">
    <img src="https://img.icons8.com/color/48/gmail-new.png" alt="Email" width="35" height="35"/>
  </a>
</div>

<!--
  Recruiter note
-->
### Employer?
> [!IMPORTANT]  
> Feel free to reach out on LinkedIn or email for a copy of my resume.

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0E7490&height=120&section=footer" alt="footer"/>
